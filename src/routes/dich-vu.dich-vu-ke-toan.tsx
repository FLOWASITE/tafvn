      ...
      <Section className="pt-0">
        ...
        <div className="mt-8 grid sm:grid-cols-3 gap-4">
          {RELATED.map((r, i) => {
            ...
            const cls =
              "group relative bg-card border border-border hover:border-accent p-6 rounded-[2px] transition-all duration-300 hover:shadow-[var(--shadow-card)] hover:-translate-y-0.5 flex flex-col gap-6 min-h-[160px]";
            return "params" in r ? (
              <Link key={r.label} to={r.to} params={r.params} className={cls}>
                {inner}
              </Link>
            ) : (
